...
C
      SUBROUTINE SET_PAGE(XOFFSET,YOFFSET,XSCALE,YSCALE)
C
      IMPLICIT LOGICAL(L)
      COMMON /GSAFLG/ LGSAF,LPAGE,LFIL,LKEEP,NPAGE,NHEAD
      COMMON /GSAFS8/ PXOFFSET,PYOFFSET,PXSCALE,PYSCALE
...
C
      SUBROUTINE INQ_PAGE(XOFFSET,YOFFSET,XSCALE,YSCALE)
C
      IMPLICIT LOGICAL(L)
      COMMON /GSAFLG/ LGSAF,LPAGE,LFIL,LKEEP,NPAGE,NHEAD
      COMMON /GSAFS8/ PXOFFSET,PYOFFSET,PXSCALE,PYSCALE
...
C
      ...
      RECT(4) = YMAX
C
      XLI=DSIZE*PXSCALE/(XMAX-XMIN)
      YLI=DSIZE*PYSCALE/(YMAX-YMIN)
      XDEL=(PXMAX-PXMIN)*XLI
      YDEL=(PYMAX-PYMIN)*YLI
      XORG=(XMAX*(PXMIN+PXOFFSET)-XMIN*(PXMAX+PXOFFSET))*XLI
      YORG=(YMAX*(PYMIN+PYOFFSET)-YMIN*(PYMAX+PYOFFSET))*YLI
      CALL OFFCLP
      RETURN
C
...
C
      IF(NMAX.GT.0) THEN
         ...
         IF(LSOFT) THEN
            IF(IFNTR.GE.2) THEN
               CALL INQLNW(WS)
               CALL SETLNW(0.08*MIN(TFLX,TFLY))
            ENDIF
         ...
         DO N=1,NMAX
            ...
            SIZE=SIZE+DX
         ENDDO
         X1=X1+(TFXX*SIZE+TFSPX*NMAX)*FACX
         Y1=Y1+(TFYX*SIZE+TFSPY*NMAX)*FACY
      ENDIF
      NMAX=0
      RETURN
      END
...
C
      ...
      ENDDO
      IF(IFNT.NE.-1) CALL SETFNT(IFNT)
      IF(IFNT.EQ.0) SIZE=SIZE-CHSP+CHW
      RETURN
      END
C
...
C
      ...
      SMRKS(3)=ANGL
      SMRKS(4)=TILT
      PXL=PXSCALE*COS(ANGL*DEG)+PYSCALE*SIN(ANGL*DEG)
      PYL=PYSCALE*COS(ANGL*DEG)-PXSCALE*SIN(ANGL*DEG)
      IMKH=NINT(HMRK*DSIZE*PYL)
      IMKW=NINT(WMRK*DSIZE*PXL)
      CALL GS_SFSTMK(IMRK,IMKH,IMKW,ANGL,TILT)
      IF(LFIL) CALL BUFFST(4,IMRK,IMKH,IMKW,ANGL,TILT)
C
      RETURN
      END
C
...
C
      ...
      PXL=PXSCALE*COS(ANGL*DEG)+PYSCALE*SIN(ANGL*DEG)
      PYL=PYSCALE*COS(ANGL*DEG)-PXSCALE*SIN(ANGL*DEG)
      ICHH =NINT(CHARS(1)*DSIZE*PYL)
      ICHW =NINT(CHARS(2)*DSIZE*PXL)
      ICHSP=NINT(CHARS(3)*DSIZE*PXL)
C
      IF(LFIL) CALL BUFFST(2,ICHH,ICHW,ICHSP,ANGL,TILT)
      IF(LSOFT) THEN
         CALL GS_SFSTCH(ICHH,ICHW,ICHSP,ANGL,TILT)
      ELSE
         CALL DVSTCH(ICHH,ICHW,ICHSP,ANGL,TILT,IND)
      ENDIF
      RETURN
      END
C
