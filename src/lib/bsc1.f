...
C
      IF(NMAX.GT.0) THEN
         CALL INQVEW(PXMIN,PXMAX,PYMIN,PYMAX,XMIN,XMAX,YMIN,YMAX)
         CALL INQ_PAGE(XOFFSET,YOFFSET,XSCALE,YSCALE)
...
C
      ...
      SMRKS(3)=ANGL
      SMRKS(4)=TILT
      IMKH =NINT(SMRKS(1)*DSIZE)
      IMKW =NINT(SMRKS(2)*DSIZE)
...
C
      ANGL =CHARS(4)
      TILT =CHARS(5)
      ICHH =NINT(CHARS(1)*DSIZE)
      ICHW =NINT(CHARS(2)*DSIZE)
      ICHSP=NINT(CHARS(3)*DSIZE)
C
