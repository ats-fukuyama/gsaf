...
C
      ...
      DIMENSION PV(3)
      DIMENSION ZVALUE1(NXDMP,NYDMP),INDEX(NXYDM)
      DIMENSION ZPLANE(NXYDM)
C
...
C      NE=INT((PX1+EPS)/DPX)
      NS=INT(PX0/DPX-EPS)
      NE=INT(PX1/DPX+EPS)
      IF(NS.LT.-NAXM) NS=-NAXM
      IF(NE.GE. NAXM) NE= NAXM-1
C
      IF (IYZ.EQ.0) THEN
         PY=YPOS
         PZ=ZPOS
