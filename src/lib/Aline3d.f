...
C
      ...
      COMMON /CPLTHD/ NF,IXMINB,IYMINB,IHXY(NXDM,NYDM),DPMX(2560,1810),
     &     NNMX(2560,2),AFACTOR
      DATA EPS/0.10/
C     
...
C
         ...
         IXB=NINT(AFACTOR*XNM)
         IYB=NINT(AFACTOR*YNM)
         IF (NX*NY.GT.2000) THEN
            IF (ZNM.GT.DPMX(IXB-IXMINB,MAX(IYB-IYMINB,1))) GOTO 300
         ELSE
            IF (ZNM.GE.DPMX(IXB-IXMINB,MAX(IYB-IYMINB,1))+10.0) GOTO 300
         ENDIF
         IF (IMOD.EQ.1) THEN
...
C
      ...
      COMMON /GLNGTH/ XL,YL,ZL,ZMIN,ZMAX,NX,NY
      COMMON /GDFN2D/ GXS,GXE,GYS,GYE
      COMMON /GTHNDT/ XHDATA(NXDMP,2),YHDATA(NYDMP,2)
