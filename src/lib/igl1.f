...
C
      ...
      CA=COS(A)
      SA=SIN(A)
      IXMKX=NINT( CHX*CA     *PXSCALE)
      IXMKY=NINT(-CHY*SIN(B) *PXSCALE)
      IYMKX=NINT( CHX*SA     *PYSCALE)
      IYMKY=NINT( CHY*COS(B) *PYSCALE)
      RETURN
      END
C
...
C
      ...
      IF(COST.GE.0.0.AND.COST.LT. 0.1) COST= 0.1
      IF(COST.LT.0.0.AND.COST.GT.-0.1) COST=-0.1
      CHY=0.166667*REAL(ICHH)/COST
      CA=COS(A)
      SA=SIN(A)
      IXCHX=NINT( CHX*CA     *PXSCALE)
      IYCHX=NINT( CHX*SA     *PYSCALE)
      IXCHY=NINT(-CHY*SIN(B) *PXSCALE)
      IYCHY=NINT( CHY*COS(B) *PYSCALE)
      IXCHSP=NINT(ICHSP*CA   *PXSCALE)
      IYCHSP=NINT(ICHSP*SA   *PYSCALE)
C
      CHY=1.0/COST
      IF(ICHH.EQ.0) THEN
         FACT=1.0
      ...
      ENDIF
      TFLY=ICHH/DSIZE      
      TFLX=ICHH/DSIZE *FACT
      TFXX= CA    *FACT *PXSCALE
      TFYX= SA    *FACT *PYSCALE
      TFXY=-SIN(B)*CHY  *PXSCALE
      TFYY= COS(B)*CHY  *PYSCALE
      TFSP= (ICHSP-ICHH)/DSIZE
      TFSPX=CA*TFSP     *PXSCALE
      TFSPY=SA*TFSP     *PYSCALE
      RETURN
      END
