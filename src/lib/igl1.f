...
C
      ...
      IXCHX=NINT( CHX*CA)
      IYCHX=NINT( CHX*SA)
      IXCHY=NINT(-CHY*SIN(B))
      IYCHY=NINT( CHY*COS(B))
      IXCHSP=NINT(ICHSP*CA)
      IYCHSP=NINT(ICHSP*SA)
C
      PXL=PXSCALE*CA+PYSCALE*SA
      PYL=PYSCALE*CA-PXSCALE*SA
      CHY=1.0/COST
      IF(ICHH.EQ.0) THEN
         FACT=1.0
      ELSE
